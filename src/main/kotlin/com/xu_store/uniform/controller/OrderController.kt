...
import com.xu_store.uniform.dto.OrderResponseDTO
...
class OrderController(
    ...
    fun getAllOrders(): ResponseEntity<List<OrderResponseDTO>> {
        ...
        val response = orders.map { OrderResponseDTO.from(it) }
        return ResponseEntity.ok(response)
    }

    // Optional: Endpoint for a user to view their own orders.
    @GetMapping("/my")
    fun getMyOrders(principal: Principal): ResponseEntity<List<OrderResponseDTO>> {
